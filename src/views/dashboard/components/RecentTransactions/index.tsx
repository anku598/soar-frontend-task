...
export function RecentTransactions({ transactions }: RecentTransactionsProps) {
  return (
    <div>
      ...
      <div className="bg-white rounded-[25px] p-0 sm:p-6 sm:min-h-[250px] ">
          ...
          {transactions.map((transaction) => (
            ...
            >
              <div className="flex items-center gap-4">
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center ${getIconBackground(transaction.type)}`}
                >
                  <img
                ...
                <div>
                  <p className="text-[#232323] font-[500] text-[14px] sm:text-[16px]">
                    {transaction.description}
                  </p>
                  <p className="text-[12px] text-[#718EBF] font-[400] sm:text-[15px]">
                    {formatDate(transaction.date)}
                  </p>
                </div>
              </div>
              <span
                className={`font-[500] text-[11px] sm:text-[16px] ${getAmountColor(transaction.type)}`}
              >
